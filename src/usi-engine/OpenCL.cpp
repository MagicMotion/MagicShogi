...
//    myprintf("finalSize_pol=%d,finalSize_val=%d,m_layers.size()=%d\n",finalSize_pol,finalSize_val,m_layers.size());
...
//          getOpenCL().m_batch_size * WINOGRAD_TILE * m_ceil * n_ceil * sizeof(net_t);
            getOpenCL().m_batch_size * WINOGRAD_TILE * m_ceil * n_ceil * sizeof(net_t) * 2;	// not enough for AobaZero 256x20b. 64x15b is ok.
...
void OpenCL_Network<net_t>::convolve1(OpenCLContext & opencl_context,
    ...
    std::cerr << "channels=" << channels << ",outputs=" << outputs << ",mergeSize=" << mergeSize << ",bufferMerge=" << bufferMerge.getInfo<CL_MEM_SIZE>() << std::endl;
