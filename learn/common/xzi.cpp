...
uint64_t XZAux::crc64(const PtrLen<char> pl) noexcept {
  return lzma_crc64(reinterpret_cast<const uint8_t *>(pl.p), pl.len, 0); }
...
void XZBase::xzwrite(DevNul *out, size_t len) const noexcept {}
...
bool XZDecode<T_IN, T_OUT>::getline(T_IN *in, T_OUT *out, size_t len_limit,
  ...
  while (true) {
    ...
    ret = lzma_code(&_strm, action);
    if (ret == LZMA_OK || ret == LZMA_STREAM_END) { continue; }
    break; }
